...
int main() {
    ...
    return 0;
}`,ke=`// Slim "Spike-like" spectrum analyzer GUI for USRP / HackRF / Pluto on CLER.
...
`,we={hello_world:v,plots:N,polyphase_channelizer:$,mass_spring_damper:E,uhd_device:ue,spike:J,adsb_receiver:h,type_conflict:oe},ve={hello_world:me,plots:xe,polyphase_channelizer:he,mass_spring_damper:ge,uhd_device:be,spike:ke,adsb_receiver:fe,type_conflict:ye},Pe=Object.keys(we),Se={toolchain:30,boot:6,stage:6,compile:30,link:24,optimize:6,store:1,launch:1},a=["toolchain","boot","stage","compile","link","optimize","store","launch"],Ce="cler.build.timings";function Re(e){switch(e.phase){case"toolchain":return{phrase:"Downloading the compiler…",target:e.total?`${d(e.bytes??0)} / ${d(e.total)} MB`:`${d(e.bytes??0)} MB`};case"boot":return{phrase:"Unpacking clang — warming the toolchain…",target:null};case"stage":return{phrase:e.detail?"Staging cler headers":"Staging cler headers…",target:e.detail??null};case"compile":return{phrase:"Compiling",target:e.detail??"the flowgraph"};case"link":return{phrase:"Persuading wasm-ld — linking against libcler_web…",target:null};case"optimize":return{phrase:"Optimizing wasm (Asyncify)…",target:null};case"store":return{phrase:"Storing the build in your browser…",target:null};case"launch":return{phrase:"Wiring runners — launching…",target:null}}}function d(e){return(e/1e6).toFixed(1)}function _(e){const r={...Se};for(const t of a){const n=e[t];n&&n>0&&(r[t]=n)}return r}function Ie(e,r,t){const n=_(t),i=a.reduce((l,p)=>l+n[p],0),c=a.slice(0,a.indexOf(e.phase)).reduce((l,p)=>l+n[p],0),o=Be(e,r,t);return o===null?null:Math.min(1,(c+o*n[e.phase])/i)}function Be(e,r,t){if(e.phase==="toolchain"&&e.total)return Math.min(1,(e.bytes??0)/e.total);const n=t[e.phase];return!n||n<=0?null:Math.min(.95,r/n)}function ze(e,r,t){if(Object.keys(t).length===0)return null;const n=_(t),i=a.slice(a.indexOf(e.phase)+1).reduce((o,l)=>o+(t[l]??0),0),c=t[e.phase]?Math.max(0,t[e.phase]-r):e.phase==="toolchain"&&e.total?(1-Math.min(1,(e.bytes??0)/e.total))*n.toolchain:0;return Math.round((i+c)/1e3)}const Te=["This build is running inside your tab — there is no server compiling anything.","cler-fg edits the .cpp itself: the file is the only source of truth, no project file, no export step.","read_dbf/write_dbf hand a block a pointer into the channel — the default path is zero-copy.","A block owns its input channels; upstream blocks only get a reference to write into.","The progress contract: return cler::Empty{} only when you actually moved a sample.","A block that fails on one input may still succeed on another — errors are retryable, not fatal.","PinnedIslands cuts the graph into contiguous islands, one pinned worker each — the pick for 2 cores.","The core is header-only, so the same flowgraph compiles down to bare metal.","Prefer read_dbf/write_dbf over push/pop: push/pop in a hot path is the classic cler mistake.","The run window is cross-origin isolated, which is the only reason pthreads work in it."];let s=null;function Ee(e){return s=e,()=>{s===e&&(s=null)}}function De(e){s?.(e)}export{Te as F,Ce as T,we as a,Re as b,Pe as c,De as d,ve as f,Ee as o,Ie as p,ze as r};
