...
// Compute basic statistics for a single block (no I/O dependencies)
struct BasicBlockStats {
    size_t total_procedures;
    size_t successful_procedures; 
    size_t failed_procedures;
    float success_rate_percent;
    float dead_time_ratio_percent;
};

template<typename ThreadingPolicy, typename... BlockRunners>
BasicBlockStats compute_block_stats(const FlowGraph<ThreadingPolicy, BlockRunners...>& fg, size_t block_index) {
    const auto& stats = fg.stats();
    if (block_index >= stats.size()) {
        return {0, 0, 0, 0.0f, 0.0f};
    }
    
    const auto& s = stats[block_index];
...
// Buffer size recommendation based on sample rate and latency requirements
[[maybe_unused]] inline size_t recommend_buffer_size(double sample_rate_hz, double max_latency_ms) {
    size_t min_samples = static_cast<size_t>(sample_rate_hz * max_latency_ms / 1000.0);
    return ceil2p2(min_samples);  // Round up to next power of 2 for efficiency
}
