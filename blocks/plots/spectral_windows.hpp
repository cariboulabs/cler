...
inline constexpr float flattop_window(float x) {
    constexpr float PI = 3.14159265358979323846f;
    return 1.0f
        - 1.93f * std::cos(2 * PI * x)
        + 1.29f * std::cos(4 * PI * x)
        - 0.388f * std::cos(6 * PI * x)
        + 0.0322f * std::cos(8 * PI * x);
}
...
inline constexpr float spectral_window_function(SpectralWindow type, float x, float beta = 8.6f) {
    ...
    switch (type) {
        case SpectralWindow::BlackmanHarris:
            return 0.35875f - 0.48829f * std::cos(2 * PI * x)
                           + 0.14128f * std::cos(4 * PI * x)
                           - 0.01168f * std::cos(6 * PI * x);
        case SpectralWindow::Hamming:
            return 0.54f - 0.46f * std::cos(2 * PI * x);
        case SpectralWindow::Hann:
            return 0.5f * (1.0f - std::cos(2 * PI * x));
