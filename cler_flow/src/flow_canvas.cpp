...
std::string FlowCanvas::GenerateCppCode() const
{
    ...
    code << "#include \"task_policies/cler_desktop_tpolicy.hpp\"\n";
    ...
    // Create block instances (CLER style - stack objects)
    code << "    // Create blocks\n";
    for (const auto& [id, node] : nodes) {
        code << "    " << node->GenerateInstantiation();
    }
    
    // Build a map of blocks to their output connections
    std::map<size_t, std::vector<Connection>> block_outputs;
    for (const auto& conn : connections) {
        block_outputs[conn.from_node_id].push_back(conn);
    }
    
    // Generate BlockRunner list
    code << "\n    // Create flowgraph with BlockRunners\n";
    code << "    auto flowgraph = cler::make_desktop_flowgraph(\n";
    
    bool first_runner = true;
    for (const auto& [id, node] : nodes) {
        if (!first_runner) code << ",\n";
        first_runner = false;
        
        code << "        cler::BlockRunner(&" << node->instance_name;
        
        // Add output connections for this block
        if (block_outputs.find(id) != block_outputs.end()) {
            for (const auto& conn : block_outputs[id]) {
                auto to_it = nodes.find(conn.to_node_id);
                if (to_it != nodes.end()) {
                    auto* to_node = to_it->second.get();
                    if (conn.to_port_index < to_node->input_ports.size()) {
                        std::string to_port = to_node->input_ports[conn.to_port_index].name;
                        code << ", &" << to_node->instance_name << ".";
                        ...
                        // Check if it's an array port (like in[0], in[1])
                        if (to_node->input_ports.size() > 1 && to_port == "in") {
                            code << "[" << conn.to_port_index << "]";
                        }
                    }
                }
            }
    ...
    code << "    flowgraph.run();\n";
    code << "\n";
    code << "    // Keep running until Ctrl+C\n";
    code << "    std::cout << \"Flowgraph running. Press Ctrl+C to stop...\" << std::endl;\n";
    code << "    while (true) {\n";
