...
path writes a PNG (zlib, detected at build time), any other extension a BMP.
Prefer PNG: most tooling, including Claude's own image reading, cannot open BMP.

`spike` exposes this on the CLI, so a script or agent can capture the trigger
scope with no clicking. It needs a real display (`DISPLAY` set), waits for the
trigger to publish a new frame before grabbing, and forces the scope visible
regardless of the saved conf.
...
present; `--capture-no-dat` skips the companion `.dat`, whose spectrogram blob
reaches tens of MB. Capture runs never write `~/.cler_spike.conf`, so they
cannot clobber hand-tuned settings.
