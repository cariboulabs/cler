...
#include "ais_sim_source.hpp"
#ifdef AIS_HAVE_HACKRF
...
// One source block, picked at runtime: HackRF, IQ file or the synthetic bay.
// File and sim are paced to real time here so the map's clocks mean something.
struct AISSourceBlock : public cler::BlockBase {
    static constexpr bool may_block = true;
    enum class Kind { HackRF, File, Sim };

    AISSourceBlock(const char* name, Kind kind, const std::string& file, double center_hz, double rate_hz,
    ...
    cler::Result<cler::Empty, cler::Error> procedure(cler::ChannelBase<std::complex<float>>* out) {
        ...
        return std::visit([&](auto& s) -> cler::Result<cler::Empty, cler::Error> {
            if constexpr (std::is_same_v<std::decay_t<decltype(s)>, std::monostate>) {
                cler::panic("AISSourceBlock: empty source");
            } else {
...
private:
    using Variant = std::variant<std::monostate, AISSimSourceBlock, FileIQSourceBlock
#ifdef AIS_HAVE_HACKRF
        , SourceHackRFBlock
#endif
    >;
    ...
    static Variant make(Kind kind, const std::string& file, double center_hz, double rate_hz, int lna, int vga, bool amp) {
        ...
        switch (kind) {
            case Kind::Sim: return Variant(std::in_place_type<AISSimSourceBlock>, "Sim ships", rate_hz, size_t{1} << 18);
...
#else
                cler::panic("ais_receiver: built without HackRF; use --sim or --file");
#endif
        }
        cler::panic("AISSourceBlock: unknown kind");
    }
